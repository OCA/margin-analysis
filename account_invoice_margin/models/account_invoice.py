...
class AccountInvoice(models.Model):
    ...
    def _compute_margin(self):
        real_invoices = self.filtered(lambda x: not isinstance(x.id, models.NewId))
                ...
                invoice.update(
                    {
                        ...
                        "margin_percent": (
                            group["price_subtotal"]
                            and group["margin_signed"] / group["price_subtotal"] * 100
                            or 0
                        ),
                    }
                )
            ...
            invoice.update(
                {
                    ...
                    "margin_percent": price_subtotal
                    and (margin_signed / price_subtotal * 100)
                    or 0,
                }
            )
...
class AccountInvoiceLine(models.Model):
    ...
    purchase_price = fields.Float(
        digits=dp.get_precision("Product Price"), string="Cost"
    )
