...
class account_invoice_line(orm.Model):
    ...
    Later, we'll be able to provide as well a view based on an invoice line SQL
    view, so user will be allowed to play with filter and group by.
    """
    _inherit = 'account.invoice.line'

    def _compute_line_values(self, cr, uid, ids,
                             field_names, arg, context=None):
        """
        Compute cost_price, cost_price in company currency and subtotal in
        company currency that will be used for margin analysis. This method is
        called by the function fields.

        Those values will be stored, so we'll be able to use them in analysis.

        WARNING !! All subtotal in company currency will be with the right
        sign. For example, if I have a customer refund, the sign will be - For
        all infos in invoice currency, we kept the standard logic => always
        positif.
        ...
        fields = ['subtotal_cost_price_company',
                  'subtotal_cost_price',
                  'subtotal_company',
                  'margin_absolute',
                  'margin_relative'
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            # The company must be the one of the invoice in case a ir.cron
            # create the invoice with admin user. We need to pass it in the
            # context as well if we use also product_price_history with this
            # module
            if obj.company_id:
            ...
            # invoice currency
            subtotal_cost_price = currency_obj.compute(
                cr, uid,
                company_currency_id,
                currency_id,
    ...
    _columns = {
        'subtotal_cost_price_company': fields.function(
            ...
            help="The cost subtotal of the line at the time of the creation "
                 "of the invoice, express in the company currency."),
        'subtotal_cost_price': fields.function(
            _compute_line_values,
            method=True,
            readonly=True,
            type='float',
        ...
        'subtotal_company': fields.function(
            ...
            store=_col_store,
            digits_compute=dp.get_precision('Account'),
            help="The subtotal (VAT excluded) of the line at the time of "
                 "the creation of the invoice, express in the company "
    ...
    def read_group(self, cr, uid, domain, fields, groupby,
        ...
        nor a sum, nor a avg, but a percentage of 2 valuesof the line computed
        as: margin_relative = margin_absolute / subtotal_company * 100"""
        if not context:
            context = {}
        _super = super(account_invoice_line, self)
        res = _super.read_group(cr, uid,
                                domain,
                                fields,
                                groupby,
                                offset=offset,
            ...
            if re.get('margin_relative', False):
                ...
                if re.get('__context', False):
                    ...
                    if subtotal_company == 0.0:
                        ...
                        margin_relative = (margin_absolute /
                                           subtotal_company * 100)
            if re.get('__context', False):
